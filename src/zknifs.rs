...
use crate::{
  ...
  errors::NovaError,
  r1cs::{
    R1CSInstance, R1CSResult, R1CSShape, ZKR1CSWitness, RelaxedR1CSInstance, ZKRelaxedR1CSWitness,
  },
  scalar_as_base,
...
impl<E: Engine> NIFS<E> where <E as traits::Engine>::CE: ZKCommitmentEngineTrait<E> {
  ...
  pub fn prove(
    ...
    (
      Self,
      (RelaxedR1CSInstance<E>, ZKRelaxedR1CSWitness<E>),
      E::Scalar,
    ),
    NovaError,
  > {
    ...
    let r_T = <E as Engine>::Scalar::random(&mut OsRng);

    // compute a commitment to the cross-term
    let (T, comm_T) = S.commit_T_zk(ck, U1, W1, U2, W2, &r_T)?;
  ...
  pub fn prove_mut(
    ...
    // compute a commitment to the cross-term
    let comm_T = S.commit_T_into_zk(ck, U1, W1, U2, W2, T, ABC_Z_1, ABC_Z_2, &r_T)?;
...
//     let (U1, W1) = cs.r1cs_instance_and_zkwitness(&shape, &ck).unwrap();
...
//     let (U2, W2) = cs.r1cs_instance_and_zkwitness(&shape, &ck).unwrap();
...
//     let mut r_W = ZKRelaxedR1CSWitness::default(shape);
...
//       |ck: &CommitmentKey<E>, I: &E::Scalar| -> (E::Scalar, R1CSInstance<E>, ZKR1CSWitness<E>) {
//         let i0 = *I;
...
//         let W = {
//           let res = ZKR1CSWitness::new(&S, vars);
//           assert!(res.is_ok());
//           res.unwrap()
//         };
//         let U = {
