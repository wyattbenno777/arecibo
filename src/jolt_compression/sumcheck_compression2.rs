...
#[derive(Clone)]
pub struct SumcheckVerifierCircuit2<F: PrimeField> {
...
impl<F: PrimeField> Circuit<F> for SumcheckVerifierCircuit2<F> {
...
impl SumcheckCompression2 {
  ...
  pub fn compress(
    ...
    let circuit = SumcheckVerifierCircuit2::<Fr> {
      polys,
      claim,
      num_rounds,
      degree_bound,
  ...
  pub fn compress_proof(
    ...
    // Convert SumcheckProof to Vec<UniPoly>
    let polys = proof.compressed_polys.iter()
      .map(|compressed_poly| compressed_poly.decompress(&claim))
      .collect();
  ...
  fn test_sumcheck_compression2() {
    ...
    for &n in &ns {
      for t in 0..num_tries {
        ...
        // Compress the proof
        let (compressed_proof, params) = SumcheckCompression2::compress_proof(
          &proof,
          claim,
          num_rounds,
          degree_bound,
