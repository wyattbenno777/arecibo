  ...
  R: rand_core::RngCore + Send,
{
  ...
  for (i, public_input) in public_inputs.iter().enumerate() {
    ...
    let d =
      (aggregate_proof_and_instance.com_f[i] - (ip_verifier_srs.g * public_input)).to_affine();
  ...
  E::G2Affine: Serialize,
{
  ...
  let final_r =
    polynomial_evaluation_product_form_from_transcript(ref_challenges_inv, r_shift, &E::Fr::ONE);
