...
use crate::{
  ...
  traits::{
    zksnark::{BatchedRelaxedR1CSSNARKTrait, RelaxedR1CSSNARKTrait},
    ...
    ZKCommitmentEngineTrait<<E1 as CurveCycleEquipped>::Secondary>,
{
  ...
  pub fn prove(
    ...
    recursive_snark: &ZKRecursiveSNARK<E1>,
  ) -> Result<Self, SuperNovaError> {
    // fold the secondary circuit's instance
    let res_secondary = crate::zknifs::NIFS::prove(
      &*pp.ck_secondary,
    ...
    // uninitialized circuits)
    let r_W_primary: Vec<ZKRelaxedR1CSWitness<E1>> = recursive_snark
      .r_W_primary
      .iter()
      .enumerate()
      .map(|(idx, r_W)| {
        r_W
          .clone()
          .unwrap_or_else(|| ZKRelaxedR1CSWitness::default(&pp[idx].r1cs_shape))
      })
      .collect::<Vec<_>>();
...
mod test {
    ...
    F: Fn(usize) -> Vec<C>,
  {
    ...
    let mut recursive_snark = ZKRecursiveSNARK::new(
      &pp,
      &test_circuits[0],
      &test_circuits[0],
      &secondary_circuit,
  ...
  #[test]
  fn test_zk_nivc_trivial_with_compression_pp() {
    const NUM_STEPS: usize = 6;
    // Curve cycle to prove on
    type E1 = Bn256EngineZKPedersen;
    ...
    type S1 = spartan::batched_zkppsnark::BatchedRelaxedR1CSSNARK<E1, EE1>;
    // SNARK for secondary NIVC
    type S2 = spartan::zkppsnark::RelaxedR1CSSNARK<Dual<E1>, EE2>;
