...
mod tests {
  ...
  use super::LinearizedR1CSSNARK;
  use crate::traits::snark::LinearizedR1CSSNARKTrait;
  ...
  fn test_pow_rs() -> Result<(), NovaError> {
    let circuit = PowCircuit {
      advice: Some(F::from(2u64)),
    };
    test_rs_with::<E, EE>(&circuit)
  }

  fn test_rs_with<E: CurveCycleEquipped, EE: EvaluationEngineTrait<E>>(
    circuit: &impl StepCircuit<E::Scalar>,
  ) -> Result<(), NovaError> {
    run_circuit::<E, EE>(circuit)
  }

  fn run_circuit<E: CurveCycleEquipped, EE: EvaluationEngineTrait<E>>(
    c: &impl StepCircuit<E::Scalar>,
  ) -> Result<(), NovaError> {
    println!("producing pp..");
    let pp = PublicParams::<E>::setup(c, &*default_ck_hint(), &*default_ck_hint());
    let z_0 = vec![E::Scalar::from(2u64), E::Scalar::from(3u64)];
    ...
    println!("snark setup..");
    let (pk, vk) =
      LinearizedR1CSSNARK::<E, EE>::setup(pp.ck.clone(), &pp.circuit_shape.r1cs_shape)?;
    println!("snark prove..");
    let (derandom_W, wit_blind) = recursive_snark.r_W.derandomize();
    let derandom_U = recursive_snark
      .r_U
      .derandomize(&E::CE::derand_key(&pp.ck), &wit_blind);
    let snark = LinearizedR1CSSNARK::<E, EE>::prove(
      &pp.ck,
      &pk,
      &pp.circuit_shape.r1cs_shape,
      &derandom_U,
    ...
    F: PrimeField,
  {
    ...
    fn synthesize<CS: ConstraintSystem<F>>(
      ...
      let mut x_pc = AllocatedNum::alloc_pre_committed(
        cs.namespace(|| "x_pc"),
        || self.advice.ok_or(SynthesisError::AssignmentMissing),
        Split::ZERO,
      )?;
    ...
    fn advice(&self) -> (Vec<F>, Vec<F>) {
      let advice = self.advice.expect("Advice should be set manually");
      (vec![advice], vec![])
    }
  }
}
