...
use crate::spartan::verify_circuit::circuit::batched::SpartanVerifyCircuit;
...
#[test]
fn test_nivc_poseidon_circuit() -> anyhow::Result<()> {
  type S1 = ipa_prover_poseidon::batched::BatchedRelaxedR1CSSNARK<E1>;
  let (snark, vk) = test_nivc_with::<_, S1>();

  let TestSNARK { snark, instance } = snark;
  let mut cs = TestConstraintSystem::<<E1 as Engine>::Scalar>::new();

  tracing::info!("synthesizing verify circuit...");
  let _ = SpartanVerifyCircuit::synthesize(cs.namespace(|| "verify"), &vk, &instance, &snark)?;
...
#[test]
fn test_nivc_poseidon() {
  type S1 = ipa_prover_poseidon::batched::BatchedRelaxedR1CSSNARK<E1>;
  test_nivc_with::<_, S1>();
}

#[test]
fn test_nivc_poseidon_cs() {
  type S1 = ipa_prover_poseidon::batched::BatchedRelaxedR1CSSNARK<E1>;
  let (snark, vk) = test_nivc_with::<_, S1>();
  ...
  let mut cs = TestConstraintSystem::<<E1 as Engine>::Scalar>::new();
  snark
    .verify_cs(cs.namespace(|| "verify cs"), &vk, &instance)
    .unwrap();
}

#[test]
fn test_nivc_standard() {
  type EE = provider::ipa_pc::EvaluationEngine<E1>;
  test_nivc_with::<E1, spartan::batched::BatchedRelaxedR1CSSNARK<E1, EE>>();
}
