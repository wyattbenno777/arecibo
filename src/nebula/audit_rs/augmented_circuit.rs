  ...
  E1: CurveCycleEquipped,
{
  ...
  prev_IC: Option<(E1::Scalar, E1::Scalar)>,
  comm_omega_prev: Option<(Commitment<E1>, Commitment<E1>)>,
  ...
  SC: AuditStepCircuit<E1::Scalar>,
{
  ...
  > {
    ...
    let prev_IC_IS = AllocatedNum::alloc(cs.namespace(|| "prev_IC_IS"), || {
      Ok(
        self
          .inputs
          .get()?
    ...
    let prev_IC_FS = AllocatedNum::alloc(cs.namespace(|| "prev_IC_FS"), || {
      Ok(
        self
          .inputs
          .get()?
    ...
    let comm_omega_prev_IS = AllocatedEmulPoint::alloc(
      ...
      self
        ...
        .map(|comm_omega_prev| comm_omega_prev.0.to_coordinates()),
      self.params.limb_width,
      self.params.n_limbs,
    )?;
    let comm_omega_prev_FS = AllocatedEmulPoint::alloc(
      ...
      self
        ...
        .as_ref()
        .and_then(|inputs| inputs.comm_omega_prev.as_ref())
        .map(|comm_omega_prev| comm_omega_prev.1.to_coordinates()),
      self.params.limb_width,
      self.params.n_limbs,
    )?;
  ...
  > {
    ...
    let mut ro_p = <Dual<E1> as Engine>::ROCircuit::new(
      ...
      5 + 2 * arity + 2 * NUM_FE_IN_EMULATED_POINT + 3,
    );
    ...
    ro_p.absorb(prev_IC_IS);
    ro_p.absorb(prev_IC_FS);
    ro_p.absorb(r_i);
    ...
    let mut ro_c = <Dual<E1> as Engine>::ROCircuit::new(
      self.ro_consts.clone(),
      1 + 1 + 1 + 3 + 3 + 1 + NIO_CYCLE_FOLD * BN_N_LIMBS, // pp + i + W + E + u + X
    );
  ...
  pub fn synthesize<CS: ConstraintSystem<E1::Scalar>>(
    ...
    let bool_is_base_case = Boolean::from(is_base_case);
    // compute incremental commitment
    let IC_i_base_case_IS =
      alloc_scalar_as_base::<Dual<E1>, _>(cs.namespace(|| "IC_0_IS"), Some(E1::Base::ZERO))?;
    let IC_i_non_base_case_IS = {
      let mut ro = <Dual<E1> as Engine>::ROCircuit::new(
        self.ro_consts.clone(),
        1 + NUM_FE_IN_EMULATED_POINT, // IC_prev + comm_omega_prev
      );

      ro.absorb(&prev_IC_IS);
      comm_omega_prev_IS.absorb_in_ro(cs.namespace(|| "absorb comm_omega_prev_IS"), &mut ro)?;

      let hash_IC_bits = ro.squeeze(cs.namespace(|| "hash_IC_bits_IS"), NUM_HASH_BITS)?;
      le_bits_to_num(cs.namespace(|| "hash_IC_IS"), &hash_IC_bits)?
    };

    let IC_i_IS = conditionally_select(
      cs.namespace(|| "select IC_IS"),
      &IC_i_base_case_IS,
      &IC_i_non_base_case_IS,
      &bool_is_base_case,
    )?;

    // compute incremental commitment for FS
    let IC_i_base_case_FS =
      alloc_scalar_as_base::<Dual<E1>, _>(cs.namespace(|| "IC_0_FS"), Some(E1::Base::ZERO))?;

    let IC_i_non_base_case_FS = {
      let mut ro = <Dual<E1> as Engine>::ROCircuit::new(
        self.ro_consts.clone(),
        1 + NUM_FE_IN_EMULATED_POINT, // IC_prev + comm_omega_prev
      );

      ro.absorb(&prev_IC_FS);
      comm_omega_prev_FS.absorb_in_ro(cs.namespace(|| "absorb comm_omega_prev_FS"), &mut ro)?;

      let hash_IC_bits = ro.squeeze(cs.namespace(|| "hash_IC_bits_FS"), NUM_HASH_BITS)?;
      le_bits_to_num(cs.namespace(|| "hash_IC_FS"), &hash_IC_bits)?
    };

    let IC_i_FS = conditionally_select(
      cs.namespace(|| "select IC_FS"),
      &IC_i_base_case_FS,
      &IC_i_non_base_case_FS,
      &bool_is_base_case,
    )?;
    ...
    let mut ro_p = <Dual<E1> as Engine>::ROCircuit::new(
      self.ro_consts.clone(),
      5 + 2 * arity + (2 * NUM_FE_IN_EMULATED_POINT + 3), // pp + IC_IS + IC_FS + i + z_0 + z_next + (U_p)
    );
    ro_p.absorb(&pp_digest);
    ro_p.absorb(&i_new);
    for e in &z_0 {
